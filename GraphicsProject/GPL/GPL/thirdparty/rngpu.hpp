...
// Uniform float func for xorwow32 generator.
INLINE_QUALIFIERS float uniform_float_xorwow32(xorwow_state32_t* state) 
{
	uint32_t a = xorwow32(state) >> 9;
...
double gaussian_float(rng_t rng, state_t * state) {
    ...
    return x * (float)sqrtf((float)(-2.0 * logf(r)) / r);
}
